...
def find_matching_frames(lead_vid_path: str, following_vids_paths: List[str],
                         seconds: int = 3, method: str = 'mse') -> List[Tuple[int, int, float]]:
    ...
    for following_vid in following_vids:
        most_similar_frames: Tuple[int, int, float] = get_most_similar_frames(lead_vid, following_vid, lead_vid_start, method)
...
def get_most_similar_frames(lead_vid: List[np.ndarray], following_vid: List[np.ndarray],
                               offset: int, method: str = 'mse') -> (int, int, float):
    if method == 'mse':
        ...
        for i, lead_frame in enumerate(lead_vid):
            ...
            for j, following_frame in enumerate(following_vid):
                # diff: float = mean_square_error(lead_frame, following_frame)
        ...
        for i, lead_frame in enumerate(lead_vid):
            print("Processing frame", i, "of", len(lead_vid) - 1)
            for j, following_frame in enumerate(following_vid):
                diff = compare_ssim(lead_frame, following_frame, multichannel=True, full=False) # Color: yes, return imgae: no
                if diff > min_diff[2]:
                    min_diff = (i + offset, j, diff)
        ...
        return get_most_similar_frames(lead_vid, following_vid, offset, 'mse')
...
# main()
print(find_matching_frames('Test/red_frame_test_1.avi', ['Test/red_frame_test_2.avi'], method='ssim'))
